...
КРИТИЧНО: клиент выбрал layout «${input.layout.id}». Сохрани его 1в1 — ту же сетку hero, порядок секций, типы блоков.
Нельзя менять на другой тип (например hero-split → hero-center). Нельзя убирать/переставлять основные секции.
Сохрани слоты data-wc-slot="hero"|data-wc-slot="image". Не удаляй эти атрибуты.
